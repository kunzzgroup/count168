    ...
    >
      ...
      {loading ? (
        ...
        <div className="space-y-2 pb-8">
          {rows.map((row, idx) => (
            <article
              key={`${row.transaction_id || idx}-${row.transaction_date}-${row.amount}`}
              className="rounded-2xl border border-slate-100 bg-white p-3 shadow-sm"
            >
              <div className="flex items-start justify-between gap-2">
                <div>
                  <p className="text-[13px] font-bold text-slate-900">{toUpperDisplay(row.transaction_type)}</p>
                  <p className="text-[11px] text-slate-500">{row.transaction_date || "—"}</p>
                </div>
              ...
              <div className="mt-2 grid grid-cols-3 gap-2 text-center">
                <HistMetric label={m.amount} value={formatHistoryMoney(row.amount)} />
