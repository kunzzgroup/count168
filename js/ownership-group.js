...
function toggleCard(groupId, event) {
    ...
    if (!isExpanded && geCurrentlyExpandedId && geCurrentlyExpandedId !== groupId) {
        cancelEdit(geCurrentlyExpandedId, true);
    }
...
function loadGroupData(groupId) {
    ...
    const editor = document.getElementById(`editor-${groupId}`);
...
function cancelEdit(groupId, forceCollapse = false) {
    document.getElementById(`card-${groupId}`).classList.remove('expanded');
    if (geCurrentlyExpandedId === groupId) geCurrentlyExpandedId = null;

    const compIdx = geGroupsData.findIndex(c => parseInt(c.id) === groupId);
    if (compIdx >= 0) {
        updateCardHeaderDisplay(groupId, parseFloat(geGroupsData[compIdx].allocated_percentage) || 0);
    }
}
...
function renderCardBodyRows(groupId) {
    const container = document.getElementById(`rows-container-${groupId}`);
    container.innerHTML = '';

    geGroupStates[groupId].rows.forEach((row, idx) => {
        container.appendChild(createRowElement(groupId, idx, row));
    });

    updateCalculations(groupId);
}

function createRowElement(groupId, idx, rowData) {
    ...
    select.addEventListener('change', () => updateRowData(groupId, idx, 'account_id', select.value));

    // Percentage input
    const input = $(div, 'percent-input');
    input.value = `${rowData.percentage}%`;
    input.id = `input-${groupId}-${idx}`;
    input.addEventListener('change', () => updateSliderFromInput(groupId, idx, input.value));

    // Slider
    const slider = $(div, 'slider');
    slider.value = rowData.percentage;
    slider.id = `slider-${groupId}-${idx}`;
    slider.addEventListener('input', () => updateInputFromSlider(groupId, idx, slider.value));
    ...
    div.addEventListener('click', (e) => {
        ...
        if (!action) return;
        switch (action) {
            case 'delete': removeRow(groupId, idx); break;
        }
    });
    ...
    div.addEventListener('dragover', (e) => {
        ...
        if (geDraggedGroupId !== groupId || geDraggedRowIdx === idx) return;
...
function addAccountRow(groupId) {
    geGroupStates[groupId].rows.push({ account_id: '', percentage: 0, role: '', user_raw_id: null, read_only: 1 });
    renderCardBodyRows(groupId);
}

function removeRow(groupId, idx) {
    geGroupStates[groupId].rows.splice(idx, 1);
    renderCardBodyRows(groupId);
}

function updateRowData(groupId, idx, field, value) {
    geGroupStates[groupId].rows[idx][field] = value;
    if (field === 'percentage') updateCalculations(groupId);
    if (field === 'account_id') {
        ...
        if (acc) {
            ...
            geGroupStates[groupId].rows[idx].read_only = 1; // default read-only for new Partnership row
        } else {
...
function updateInputFromSlider(groupId, idx, value) {
    const pct = parseFloat(value) || 0;
    document.getElementById(`input-${groupId}-${idx}`).value = `${pct}%`;
    applySliderBackground(document.getElementById(`slider-${groupId}-${idx}`));
    geGroupStates[groupId].rows[idx].percentage = pct;
    updateCalculations(groupId);
}

function updateSliderFromInput(groupId, idx, value) {
    let pct = parseFloat(value.replace('%', ''));
    if (isNaN(pct)) pct = 0;
    pct = Math.max(0, Math.min(100, pct));

    document.getElementById(`slider-${groupId}-${idx}`).value = pct;
    document.getElementById(`input-${groupId}-${idx}`).value = `${pct}%`;
    applySliderBackground(document.getElementById(`slider-${groupId}-${idx}`));
    geGroupStates[groupId].rows[idx].percentage = pct;
    updateCalculations(groupId);
...
function tweakPercentage(groupId, idx, delta) {
    const newPct = Math.max(0, Math.min(100, geGroupStates[groupId].rows[idx].percentage + delta));
    document.getElementById(`slider-${groupId}-${idx}`).value = newPct;
    updateInputFromSlider(groupId, idx, newPct);
}
...
function updateCalculations(groupId) {
    ...
    updateCardHeaderDisplay(groupId, total);

    const remaining = 100 - total;
    const footerRm = document.getElementById(`footer-remain-${groupId}`);
    const warningBadge = document.getElementById(`warning-${groupId}`);
...
function updateCardHeaderDisplay(groupId, total) {
    const remainEl = document.getElementById(`header-remain-${groupId}`);
    const pctEl = document.getElementById(`header-percent-${groupId}`);
    const barEl = document.getElementById(`header-bar-${groupId}`);
...
function _updateBulkBar() {
    ...
    if (geSelectedGroupIds.size === 0) {
        if (bar) bar.classList.remove('own-bulk-bar-visible');
        return;
    }

    const isGroupView = geActiveGroupFilter !== null;
    ...
    document.getElementById('own-bulk-count').textContent = geSelectedGroupIds.size;
...
function _clearSelection() {
    ...
    if (geSelectionMode) _exitSelectionMode();
}
...
function _bulkJoinGroup() {
    ...
    const requests = ids.map(groupId =>
...
/** Recalculates geUniqueGroupIds from the full (unfiltered) company list. */
function _rebuildGroupIds() {
    geUniqueGroupIds = [...new Set(
        geAllGroupIds
            .map(c => c.group_id)
            .filter(g => g && g.trim() !== '')
    )].sort();
...
/**
 * Applies the geActiveGroupFilter to geAllGroupIds, sets geGroupsData,
 * then triggers a card render.
 *  geActiveGroupFilter === null  → show independent companies (no group_id)
 *  geActiveGroupFilter === 'G1'  → show companies in group G1
 */
function _applyGroupFilter() {
    if (geActiveGroupFilter === null) {
        // Independent: companies with no group
        geGroupsData = geAllGroupIds.filter(c => !c.group_id || c.group_id.trim() === '');
    } else {
        geGroupsData = geAllGroupIds.filter(c =>
            c.group_id && c.group_id.toLowerCase() === geActiveGroupFilter.toLowerCase()
        );
    }
    renderGroupCards();
}
...
function _renderGroupFilterBar() {
    ...
    geUniqueGroupIds.forEach(gid => {
        ...
        btn.className = 'own-gfb-btn' + (geActiveGroupFilter === gid ? ' active' : '');
        btn.dataset.group = gid;

        // Count companies in this group
        const count = geAllGroupIds.filter(c =>
...
function joinGroupGroup(groupId, groupId, companyName) {
    fetch('api/ownership/update_company_group_api.php', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ company_id: groupId, group_id: groupId })
...
function linkExternalPartner(groupId, event, forceType = '') {
    const loginIdInput = document.getElementById(`partner-login-${groupId}`);
...
function showConflictModal(groupId, event, data) {
    ...
    clone.querySelector('[data-action="choose-login"]').addEventListener('click', () => {
        closeModal();
        linkExternalPartner(groupId, event, 'login');
    });

    clone.querySelector('[data-action="choose-group"]').addEventListener('click', () => {
        closeModal();
        linkExternalPartner(groupId, event, 'group');
    });
