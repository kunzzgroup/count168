...
function buildBankActionCellHtml(processId, status, hasTransactions, issueFlag, maintenanceResendPending) {
    const showResent = !!maintenanceResendPending;
    const resentBtn = showResent
        ? '<button type="button" class="bank-resent-btn" onclick="resentBankProcessAccountingDue(' + processId + ')" title="Restore to Accounting Due (after Maintenance delete)">Resent</button>'
        : '';
    const actionButtons = '<button class="edit-btn" onclick="editProcess(' + processId + ')" aria-label="Edit" title="Edit"><img src="images/edit.svg" alt="Edit" /></button>' +
        buildBankRemarkActionButton(processId) + resentBtn;
...
async function resentBankProcessAccountingDue(processId) {
    const id = parseInt(processId, 10);
    if (!id) return;
    if (!confirm('Resent this process into Accounting Due?\n\nThe posting calculation rules are unchanged; only the “already posted” mark for the deleted maintenance transaction will be cleared.')) {
        return;
    }
    try {
        ...
        if (result.success) {
            ...
            showNotification(result.message || 'Restored to Accounting Due', 'success');
            ...
            showNotification(result.message || 'Resent failed', 'danger');
        }
    } catch (err) {
        console.error('resentBankProcessAccountingDue:', err);
        showNotification('Request failed: ' + (err.message || 'Network error'), 'danger');
    }
}
window.resentBankProcessAccountingDue = resentBankProcessAccountingDue;
