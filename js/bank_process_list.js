...
function deleteAccountingInboxSelected() {
    ...
    if (deleteChecked.length > 0) {
        pairs = Array.from(deleteChecked).map(function (cb) { return trToDismissPair(cb.closest('tr')); }).filter(Boolean);
    } else {
        // 用户常只勾选左侧「入账」复选框：与 Transaction 一致，将该行视为要从 Accounting Due 移除的目标
        const postChecked = tbody.querySelectorAll('.process-accounting-inbox-row-cb:checked:not([disabled])');
        pairs = Array.from(postChecked).map(function (cb) { return trToDismissPair(cb.closest('tr')); }).filter(Boolean);
    }
    const seen = {};
