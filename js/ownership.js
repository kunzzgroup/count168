...
function _selectGroupFilter(groupId) {
    ...
    // ── Group Earnings Panel ──
    if (activeGroupFilter !== null) {
        fetchGroupEarnings(activeGroupFilter);
    } else {
        _hideGroupEarningsPanel();
    }
}
...
// Helper: format number with commas and 2 dp (e.g. 12345.6 → "12,345.60")
function _geFormatNum(n) {
...
// Helper: color class
function _geColorClass(n) {
    if (n > 0) return 'ge-positive';
    if (n < 0) return 'ge-negative';
    return '';
}

// Default date helpers
function _geDefaultDateFrom() {
    const d = new Date();
    return `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, '0')}-01`;
}
function _geDefaultDateTo() {
    const d = new Date();
...
function _hideGroupEarningsPanel() {
    const wrap = document.getElementById('own-group-earnings-panel');
    if (wrap) {
...
function fetchGroupEarnings(groupId, dateFrom = null, dateTo = null) {
    ...
    // Build or reuse the panel element
    if (_geCurrentGroupId !== groupId || !_gePanel) {
        ...
        if (!tpl) return;
        const clone = tpl.content.cloneNode(true);
        const panel = clone.querySelector('.own-ge-panel');

        // Set group badge
        panel.querySelector('[data-bind="group-id-badge"]').textContent = groupId;

        // Set default dates
        const dfInput = panel.querySelector('[data-bind="date-from"]');
        const dtInput = panel.querySelector('[data-bind="date-to"]');
        dfInput.value = dateFrom || _geDefaultDateFrom();
        dtInput.value = dateTo   || _geDefaultDateTo();

        // Refresh button
        panel.querySelector('[data-action="refresh-earnings"]').addEventListener('click', () => {
            fetchGroupEarnings(groupId, dfInput.value, dtInput.value);
        });

        wrap.appendChild(panel);
        wrap.style.display = 'block';
        _gePanel = wrap.querySelector('.own-ge-panel');
        _geCurrentGroupId = groupId;
    } else {
        // Reuse existing panel — update dates if provided
        if (dateFrom) _gePanel.querySelector('[data-bind="date-from"]').value = dateFrom;
        if (dateTo)   _gePanel.querySelector('[data-bind="date-to"]').value = dateTo;
    }

    // Read dates from the panel inputs (always up-to-date)
    const df = _gePanel.querySelector('[data-bind="date-from"]').value || _geDefaultDateFrom();
    const dt = _gePanel.querySelector('[data-bind="date-to"]').value   || _geDefaultDateTo();

    // Show loading, hide content
    const loadingEl = _gePanel.querySelector('[data-bind="ge-loading"]');
    const contentEl = _gePanel.querySelector('[data-bind="ge-content"]');
    ...
    fetch(`api/ownership/get_group_earnings_api.php?group_id=${encodeURIComponent(groupId)}&date_from=${df}&date_to=${dt}`)
        ...
        .then(res => {
            if (res.status !== 'success') {
                showToast(res.message || 'Failed to load group earnings', 'error');
        ...
        .catch(err => {
            console.error(err);
            showToast('Error loading group earnings', 'error');
            if (loadingEl) loadingEl.style.display = 'none';
        });
}

/**
 * Renders the earnings content into the already-mounted panel DOM.
 */
function _renderGroupEarningsContent(panel, data) {
    const loadingEl = panel.querySelector('[data-bind="ge-loading"]');
    const contentEl = panel.querySelector('[data-bind="ge-content"]');
    const summaryEl = panel.querySelector('[data-bind="ge-summary"]');
    const rowsEl    = panel.querySelector('[data-bind="ge-rows"]');
    const emptyEl   = panel.querySelector('[data-bind="ge-empty"]');
    ...
    if (summaryEl) {
        ...
        totalWrap.className = 'own-ge-summary-total';
        const totalLabel = document.createElement('span');
        totalLabel.className = 'own-ge-summary-total-label';
        totalLabel.textContent = 'Group Net Profit';
        const totalVal = document.createElement('span');
        totalVal.className = 'own-ge-summary-total-val ' + _geColorClass(data.total_profit);
        totalVal.textContent = _geFormatNum(data.total_profit);
        totalWrap.appendChild(totalLabel);
        totalWrap.appendChild(totalVal);
        summaryEl.appendChild(totalWrap);

        // Per-company chips
        (data.companies || []).forEach(c => {
            const chip = document.createElement('div');
            chip.className = 'own-ge-company-chip';
            chip.innerHTML = `
                <span class="own-ge-chip-name">${c.name}</span>
                <span class="own-ge-chip-profit ${_geColorClass(c.profit)}">${_geFormatNum(c.profit)}</span>`;
    ...
    const rowTpl = document.getElementById('tpl-ge-shareholder-row');
    if (!rowTpl || !rowsEl) return;

    shareholders.forEach(sh => {
        const frag  = rowTpl.content.cloneNode(true);
        const rowEl = frag.querySelector('.own-ge-row');

        // Avatar (first letter of name)
        const avatar = rowEl.querySelector('[data-bind="sh-avatar"]');
        if (avatar) avatar.textContent = (sh.name || '?').charAt(0).toUpperCase();

        const nameEl = rowEl.querySelector('[data-bind="sh-name"]');
        if (nameEl) nameEl.textContent = sh.name || sh.account_id;

        const loginEl = rowEl.querySelector('[data-bind="sh-login"]');
        if (loginEl) loginEl.textContent = sh.login || sh.account_id;

        const earnEl = rowEl.querySelector('[data-bind="sh-earnings"]');
        if (earnEl) {
            earnEl.textContent = _geFormatNum(sh.total_earnings);
            earnEl.className = 'own-ge-sh-earn ' + _geColorClass(sh.total_earnings);
        }

        // Breakdown rows
        const bdrRowsEl = rowEl.querySelector('[data-bind="sh-breakdown-rows"]');
        if (bdrRowsEl && Array.isArray(sh.companies)) {
            sh.companies.forEach(comp => {
                const bdr = document.createElement('div');
                bdr.className = 'own-ge-breakdown-row';
                bdr.innerHTML = `
                    <div class="own-ge-bdr-company">${comp.company_name}</div>
                    <div class="own-ge-bdr-profit ${_geColorClass(comp.profit)}">${_geFormatNum(comp.profit)}</div>
                    <div class="own-ge-bdr-pct"><span class="own-ge-pct-pill">${comp.pct}%</span></div>
                    <div class="own-ge-bdr-earn ${_geColorClass(comp.earnings)}">${_geFormatNum(comp.earnings)}</div>`;
                bdrRowsEl.appendChild(bdr);
            });
        }

        // Details toggle
        const expandBtn      = rowEl.querySelector('[data-action="toggle-breakdown"]');
        const breakdownPanel = rowEl.querySelector('[data-bind="sh-breakdown"]');
        if (expandBtn && breakdownPanel) {
            expandBtn.addEventListener('click', (e) => {
                e.stopPropagation();
                const isOpen = breakdownPanel.style.display !== 'none';
                breakdownPanel.style.display = isOpen ? 'none' : 'block';
