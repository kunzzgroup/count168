/**
 * 全站 Pjax：无刷新切换、悬停预取(100ms)、2分钟缓存、2px 进度条(点击即 30%)
 */
(function () {
    'use strict';
    ...
    var HOVER_PRELOAD_DELAY_MS = 100;
    var CACHE_MAX = 25;
    var pjaxCache = {};
    var progressTimer = null;
    var hoverPreloadTimer = null;
    var hoverPreloadUrl = null;
    ...
    function getLoadingBar() {
        ...
        style.textContent = '#' + LOADING_BAR_ID + '{position:fixed;top:0;left:0;width:0;height:2px;background:#e53935;z-index:99999;transition:width .1s ease-out,opacity .12s ease-out;}' +
    ...
    function showLoadingBar(instant30) {
        stopFakeProgress();
        var bar = getLoadingBar();
        bar.style.height = '2px';
        bar.style.width = instant30 ? '30%' : '0%';
        bar.style.opacity = '1';
        bar.classList.remove('done');
        bar.offsetHeight;
        if (instant30) {
            var n = 30;
            progressTimer = setInterval(function () { n += 3; if (n >= 92) { n = 92; stopFakeProgress(); } bar.style.width = n + '%'; }, 40);
        } else {
            var n = 0;
            progressTimer = setInterval(function () { n += 4; if (n >= 88) { n = 88; stopFakeProgress(); } bar.style.width = n + '%'; }, 35);
        }
    }
    ...
    function setCache(url, data) {
        ...
        pjaxCache[url] = { mainHtml: data.mainHtml, title: data.title, doc: data.doc, ts: Date.now() };
    }

    function getCache(url) {
        var entry = pjaxCache[url];
    ...
    function clearHoverPreload() {
        if (hoverPreloadTimer) { clearTimeout(hoverPreloadTimer); hoverPreloadTimer = null; }
        hoverPreloadUrl = null;
    }

    function prefetch(url) {
        url = normalizeUrl(url);
        if (shouldFullLoad(url)) return;
    ...
    function onNavLinkHover(url) {
        if (!url) return;
        url = normalizeUrl(url);
        if (shouldFullLoad(url) || getCache(url)) return;
        clearHoverPreload();
        hoverPreloadUrl = url;
        hoverPreloadTimer = setTimeout(function () {
            hoverPreloadTimer = null;
            if (hoverPreloadUrl === url) prefetch(url);
    ...
    document.addEventListener('mouseenter', function (e) {
        ...
        if (url) onNavLinkHover(url);
    }, true);

    document.addEventListener('mouseleave', function (e) {
        var from = e.target.closest && e.target.closest('.informationmenu-section-title[data-page], a.submenu-item[href*=".php"]');
        if (from) onNavLinkLeave();
    }, true);
