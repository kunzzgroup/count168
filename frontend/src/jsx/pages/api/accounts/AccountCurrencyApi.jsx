/**
 * Migrated from legacy/api/accounts/account_currency_api.php
 * UI placeholder - replace with full React. Backend: Spring Boot /api/legacy/*
 */
import { useEffect, useState } from "react";
import axios from "axios";

export default function AccountCurrencyApi() {
  ...
  useEffect(() => {
    let cancelled = false;
    axios
      .get("/api/legacy/stub", { params: { php: "api/accounts/account_currency_api.php" } })
  ...
  return (
    <section style={{ padding: "1rem", maxWidth: 960, margin: "0 auto" }}>
      <h1 style={{ fontSize: "1.25rem" }}>AccountCurrencyApi</h1>
      <p style={{ color: "#555", fontSize: 14 }}>
        Original PHP: <code>legacy/api/accounts/account_currency_api.php</code>
      </p>
      <p style={{ fontSize: 14 }}>
        React route: <code>/app/api/accounts/account_currency_api</code>
      </p>
      <hr />
