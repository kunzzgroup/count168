/**
 * Migrated from legacy/api/transactions/contra_reject_api.php
 * UI placeholder - replace with full React. Backend: Spring Boot /api/legacy/*
 */
import { useEffect, useState } from "react";
import axios from "axios";

export default function ContraRejectApi() {
  ...
  useEffect(() => {
    let cancelled = false;
    axios
      .get("/api/legacy/stub", { params: { php: "api/transactions/contra_reject_api.php" } })
  ...
  return (
    <section style={{ padding: "1rem", maxWidth: 960, margin: "0 auto" }}>
      <h1 style={{ fontSize: "1.25rem" }}>ContraRejectApi</h1>
      <p style={{ color: "#555", fontSize: 14 }}>
        Original PHP: <code>legacy/api/transactions/contra_reject_api.php</code>
      </p>
      <p style={{ fontSize: 14 }}>
        React route: <code>/app/api/transactions/contra_reject_api</code>
      </p>
      <hr />
