/**
 * Migrated from legacy/api/transactions/get_accounts_api.php
 * UI placeholder - replace with full React. Backend: Spring Boot /api/legacy/*
 */
import { useEffect, useState } from "react";
import axios from "axios";

export default function GetAccountsApi() {
  ...
  useEffect(() => {
    let cancelled = false;
    axios
      .get("/api/legacy/stub", { params: { php: "api/transactions/get_accounts_api.php" } })
  ...
  return (
    <section style={{ padding: "1rem", maxWidth: 960, margin: "0 auto" }}>
      <h1 style={{ fontSize: "1.25rem" }}>GetAccountsApi</h1>
      <p style={{ color: "#555", fontSize: 14 }}>
        Original PHP: <code>legacy/api/transactions/get_accounts_api.php</code>
      </p>
      <p style={{ fontSize: 14 }}>
        React route: <code>/app/api/transactions/get_accounts_api</code>
      </p>
      <hr />
