/**
 * Migrated from legacy/api/ownership/get_owners_api.php
 * UI placeholder - replace with full React. Backend: Spring Boot /api/legacy/*
 */
import { useEffect, useState } from "react";
import axios from "axios";

export default function GetOwnersApi() {
  ...
  useEffect(() => {
    let cancelled = false;
    axios
      .get("/api/legacy/stub", { params: { php: "api/ownership/get_owners_api.php" } })
  ...
  return (
    <section style={{ padding: "1rem", maxWidth: 960, margin: "0 auto" }}>
      <h1 style={{ fontSize: "1.25rem" }}>GetOwnersApi</h1>
      <p style={{ color: "#555", fontSize: 14 }}>
        Original PHP: <code>legacy/api/ownership/get_owners_api.php</code>
      </p>
      <p style={{ fontSize: 14 }}>
        React route: <code>/app/api/ownership/get_owners_api</code>
      </p>
      <hr />
