...
function collectGridRows(root) {
  const matRows = Array.from(
    root.querySelectorAll("mat-row, mat-header-row, mat-footer-row"),
  );
  if (matRows.length) return matRows;

  const roleRows = Array.from(root.querySelectorAll('[role="row"]')).filter((row) => {
    // Prefer leaf data rows (cells are direct-ish descendants), skip outer shells.
    return row.querySelectorAll(
      '[role="gridcell"], [role="columnheader"], [role="rowheader"], mat-cell, mat-header-cell, mat-footer-cell',
    ).length;
  });
  return roleRows;
}
...
function collectRowCells(row) {
  const matCells = Array.from(
    row.querySelectorAll(":scope > mat-cell, :scope > mat-header-cell, :scope > mat-footer-cell"),
  );
  if (matCells.length) return matCells;
  ...
  return Array.from(
    row.querySelectorAll(
      "mat-cell, mat-header-cell, mat-footer-cell, [role='gridcell'], [role='columnheader'], [role='rowheader']",
    ),
  ).filter((cell) => cell.closest('[role="row"], mat-row, mat-header-row, mat-footer-row') === row);
}
