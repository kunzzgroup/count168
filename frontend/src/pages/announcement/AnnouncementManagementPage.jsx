...
import { announcementApi, getJson, postForm } from '../../lib/announcementApi.js'
...
export default function AnnouncementManagementPage() {
  ...
  const loadData = async () => {
    try {
      ...
      const [announcementRes, maintenanceRes] = await Promise.all([
        getJson(announcementApi.announcementList),
        getJson(announcementApi.maintenanceList),
      ])
  ...
  const submitAnnouncement = async (event) => {
    ...
    try {
      setSubmitting(true)
      await postForm(announcementApi.announcementCreate, { title, content })
  ...
  const submitMaintenance = async (event) => {
    ...
    try {
      setSubmitting(true)
      await postForm(announcementApi.maintenanceCreate, { content })
  ...
  const deleteAnnouncement = async (id) => {
    ...
    try {
      setDeletingId(id)
      await postForm(announcementApi.announcementDelete, { id })
  ...
  const deleteMaintenance = async (id) => {
    ...
    try {
      setDeletingId(id)
      await postForm(announcementApi.maintenanceDelete, { id })
  ...
  const saveAnnouncementEdit = async (event) => {
    ...
    try {
      setSavingEdit(true)
      await postForm(announcementApi.announcementUpdate, {
  ...
  const saveMaintenanceEdit = async (event) => {
    ...
    try {
      setSavingEdit(true)
      await postForm(announcementApi.maintenanceUpdate, {
          ...
          <article className="panel">
            ...
            {maintenanceExists ? (
              <div className="message warn">
                <strong>Notice:</strong> Maintenance content already exists. Please delete the existing content before
                creating a new one.
              </div>
            ) : null}
            ...
            <form onSubmit={saveAnnouncementEdit} className="entity-form">
              <label>
                ...
                <input
                  ...
                  onChange={(event) =>
                    setEditingAnnouncement((prev) => (prev ? { ...prev, title: event.target.value } : prev))
                  }
                  maxLength={500}
                  required
                />
              ...
              <label>
                ...
                <textarea
                  ...
                  onChange={(event) =>
                    setEditingAnnouncement((prev) => (prev ? { ...prev, content: event.target.value } : prev))
                  }
                  rows={8}
                  required
                />
            ...
            <form onSubmit={saveMaintenanceEdit} className="entity-form">
              <label>
                ...
                <textarea
                  ...
                  onChange={(event) =>
                    setEditingMaintenance((prev) => (prev ? { ...prev, content: event.target.value } : prev))
                  }
                  rows={8}
                  required
                />
