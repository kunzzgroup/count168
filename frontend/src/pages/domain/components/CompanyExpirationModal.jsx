...
export default function CompanyExpirationModal({ companies, onClose, lang = "en" }) {
  ...
  return (
    <DomainModalPortal>
      <div className="fixed inset-0 z-[50002] bg-black/50 backdrop-blur-[4px]" style={{ display: "block" }}
        ...
        <h2 className="m-0 w-full border-b border-slate-200 bg-slate-50 px-[clamp(22px,1.67vw,32px)] py-[clamp(10px,1.04vw,20px)] text-[clamp(14px,1.25vw,24px)] font-bold text-slate-800">{t("companyExpirationStatus")}</h2>
                ...
                return (
                  <div key={company.company_id} className="mb-2 flex items-center justify-between rounded-lg border border-gray-200 bg-white px-[clamp(10px,1.04vw,16px)] py-[clamp(8px,0.83vw,12px)] transition-all hover:bg-gray-50 hover:shadow-[0_2px_4px_rgba(0,0,0,0.05)]">
                    <div className="flex flex-col gap-1">
                      ...
                      <div className="text-[clamp(8px,0.625vw,12px)] font-bold text-slate-500">{t("expirationPrefix")}{formattedDate}</div>
                    </div>
