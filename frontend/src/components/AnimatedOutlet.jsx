...
/**
 * Keeps visited routes mounted (display:none) so sidebar hops reuse DOM + in-memory data.
 * While a lazy chunk loads, the previous page stays visible — no blank main area.
 */
  ...
  return (
    <main className="ec-page-shell" aria-live="polite">
      {Array.from(cache.entries()).map(([path, element]) => (
        <div
          key={path}
          className="ec-page-shell__content"
          hidden={path !== visiblePath}
