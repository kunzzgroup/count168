<?php
// 会话超时配置（单位：秒，这里为 1 小时，与 index.php SPA 壳保持一致）
$sessionTimeout = 3600;
$cookieOptions = [
...
try {
    ...
    if ($login_role === 'member') {
        ...
        if ($account) {
            ...
            echo json_encode(['status' => 'success', 'redirect' => 'index.php?r=/member']);
            exit;
        } else {
    ...
    if ($user) {
        ...
        if ($needs_secondary_password) {
            ...
            echo json_encode(['status' => 'success', 'redirect' => 'index.php?r=/dashboard']);
        }
        exit;
