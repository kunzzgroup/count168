...
- Dev origin：`http://127.0.0.1:5173`（以本机 Vite 为准）
- 路径：`spaPath(pageKey)` → `/{path}/{uuid}`，定义在 `frontend/src/utils/routing/pageRoutes.js`

示例（以源码 UUID 为准，若路由表变更以文件为准）：

- dashboard → `/dashboard/f758d9be-bed3-4576-87c0-7c4c39331b87`
...
```text
按 e2e-pipeline skill 执行：圈定本次改动相关路由 → Playwright MCP 冒烟 → Review → 输出 Bug 清单后停步等我确认再修。不要调用外部 Codex 扩展。
```
