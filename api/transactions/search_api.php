    ...
    if (searchApiTxnHasCurrencyId($pdo)) {
        ...
        $sql = "SELECT t.account_id, IFNULL(t.currency_id, 0) AS currency_id,
                 SUM(CASE WHEN $wlDateExpr < ? THEN (
                    CASE 
                        WHEN t.transaction_type = 'WIN' AND (t.description LIKE 'Process: %' OR t.description LIKE 'Inactive Compensation Sell Price%') THEN ROUND(t.amount, 2)
                        WHEN t.transaction_type = 'LOSE' AND (t.description LIKE 'Process: %' OR t.description LIKE 'Inactive Compensation Sell Price%') THEN -ROUND(t.amount, 2)
                        WHEN t.transaction_type = 'WIN' AND ((t.description NOT LIKE 'Process: %' AND t.description NOT LIKE 'Inactive Compensation Sell Price%') OR t.description IS NULL) THEN -ROUND(t.amount, 2)
                        WHEN t.transaction_type = 'LOSE' AND ((t.description NOT LIKE 'Process: %' AND t.description NOT LIKE 'Inactive Compensation Sell Price%') OR t.description IS NULL) THEN ROUND(t.amount, 2)
                        ELSE 0 
                    END
    ...
    if (searchApiTxnHasCurrencyId($pdo)) {
        ...
        $sql = "SELECT COALESCE(SUM(CASE WHEN t.transaction_type = 'WIN' THEN ROUND(t.amount, 2) WHEN t.transaction_type = 'LOSE' THEN -ROUND(t.amount, 2) ELSE 0 END), 0) as total
                ...
                WHERE t.company_id = ? AND t.account_id = ? AND $wlDateExpr BETWEEN ? AND ?
                  ...
                  AND (t.description LIKE 'Process: %' OR t.description LIKE 'Inactive Compensation Sell Price%')"
            . $wlFutureGuard;
        $stmt = $pdo->prepare($sql);
        ...
        $sql = "SELECT COALESCE(SUM(CASE WHEN t.transaction_type = 'WIN' THEN -ROUND(t.amount, 2) WHEN t.transaction_type = 'LOSE' THEN ROUND(t.amount, 2) ELSE 0 END), 0) as total
                ...
                WHERE t.company_id = ? AND t.account_id = ? AND $wlDateExpr BETWEEN ? AND ?
                  AND t.currency_id = ? AND t.transaction_type IN ('WIN', 'LOSE')
                  AND ((t.description NOT LIKE 'Process: %' AND t.description NOT LIKE 'Inactive Compensation Sell Price%') OR t.description IS NULL)"
            . $wlFutureGuard;
        $stmt = $pdo->prepare($sql);
        $stmt->execute([$company_id, $account_id, $date_from, $date_to, $currency_id]);
        $win_loss += $stmt->fetchColumn();
