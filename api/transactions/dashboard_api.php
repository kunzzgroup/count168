...
try {
    ...
    foreach ($roles as $role) {
        ...
        if ($filter_currency_code !== null) {
            // Find currency ID for the code
            $curr_id = array_search($filter_currency_code, $currency_map);
            if ($curr_id !== false) {
                $currency_filter = " AND currency_id = ?";
        ...
        $sql = "SELECT COALESCE(SUM(dcd.processed_amount), 0)
                ...
                WHERE dc.company_id = ?
                  ...
                  AND dc.capture_date < ?" . $currency_filter;
        ...
        if ($hasTransactionCurrency) {
            ...
            $sql = "SELECT COALESCE(SUM(CASE 
                    ...
                    WHERE t.company_id = ?
                      ...
                      AND t.transaction_date < ?" . $currency_filter . $clearFilter . $contraApproval;
            $bf_stmt = $pdo->prepare($sql);
            $bf_stmt->execute(array_merge([$company_id], $account_ids, [$date_from_db], $currency_params));
            $total_bf += (float)$bf_stmt->fetchColumn();
                ...
                if ($rateCheck && $rateCheck->rowCount() > 0) {
                    $sql = "SELECT COALESCE(SUM(CASE
                            ...
                            WHERE h.company_id = ?
                              ...
                              AND h.transaction_date < ?" . $currency_filter;
                    $bf_stmt = $pdo->prepare($sql);
                    $bf_stmt->execute(array_merge([$company_id], $account_ids, [$date_from_db], $currency_params));
                    $total_bf += (float)$bf_stmt->fetchColumn();
                }
        ...
        $sql = "SELECT DATE(dc.capture_date) as date, 
                ...
                WHERE dc.company_id = ?
                  AND dcd.account_id IN ($ids_placeholder)
                  AND dc.capture_date BETWEEN ? AND ?" . $currency_filter . "
        ...
        if ($hasTransactionCurrency) {
            ...
            $sql = "SELECT DATE(t.transaction_date) as date,
                    ...
                    WHERE t.company_id = ?
                      AND t.account_id IN ($ids_placeholder)
                      AND t.transaction_date BETWEEN ? AND ?" . $currency_filter . $clearFilter . $contraApproval . "
            ...
            $sql = "SELECT DATE(t.transaction_date) as date,
                           COALESCE(SUM(CASE 
                               WHEN transaction_type IN ('PAYMENT', 'RECEIVE', 'CLAIM', 'CONTRA', 'CLEAR', 'RATE') THEN amount
                               ELSE 0
                           END), 0) as cr_dr
                    FROM transactions t
                    WHERE t.company_id = ?
            ...
            try {
                ...
                if ($rateCheck && $rateCheck->rowCount() > 0) {
                    $sql = "SELECT DATE(h.transaction_date) as date,
                            ...
                            WHERE h.company_id = ?
                              AND e.account_id IN ($ids_placeholder)
                              AND h.transaction_date BETWEEN ? AND ?" . $currency_filter . "
