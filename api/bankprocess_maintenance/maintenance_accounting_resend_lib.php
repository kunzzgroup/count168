<?php
/**
 * Bank Process Maintenance：维护页删除 transactions 后，记录可「Resent」恢复 Accounting Due 的元数据。
 * 不修改入账算法，仅清除对应的 process_accounting_posted，让 inbox 再次按原规则出现该行。
 */
...
if (!function_exists('bmp_resolveProcessAccountingPostedId')) {
    /**
     * 根据被删 transaction 反查应对应清除的 process_accounting_posted.id
     */
...
if (!function_exists('bmp_recordResendPendingForTransactionIds')) {
    /**
     * 在删除 transactions 之前调用：写入待 Resent 记录（按 pap 主键或 fallback 唯一键去重）。
     */
