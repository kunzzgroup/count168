...
/**
 * 结束 Resend relax 会话（入账成功或 Accounting Due Dismiss 后调用），恢复常规 monthly 排期。
 *
 * @return bool true if relax was active and cleared
 */
if (!function_exists('bmp_clearBankProcessAccountingResendRelaxState')) {
    function bmp_clearBankProcessAccountingResendRelaxState(PDO $pdo, int $companyId, int $bankProcessId): bool
    {
        if (!bmp_resend_tableHasColumn($pdo, 'bank_process', 'accounting_resend_relax_created_floor')) {
            return false;
        }
