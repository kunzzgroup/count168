<?php
/**
 * Bank Process List：将曾在 Maintenance 中删除入账交易后的 Process 恢复进 Accounting Due。
 * 做法：删除对应的 process_accounting_posted 记录（及待办行），不改动入账计算逻辑本身。
 */
...
try {
    ...
    if (empty($pending)) {
        throw new Exception('当前没有可通过 Resent 恢复的维护删除记录');
    }
    ...
    $pdo->commit();
    jsonResponse(true, '已恢复：该 Process 可再次进入 Accounting Due', [
