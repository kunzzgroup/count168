...
function toggleFormatDomainAutoDisplayAccountId(string $rawAccountId): string {
...
function toggleResolveAccountPk(PDO $pdo, int $companyId, int $numericPk, string $accountCode, string $displayCode): int {
    if ($numericPk > 0) {
    ...
    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
        ...
        $display = strtoupper(trim(toggleFormatDomainAutoDisplayAccountId((string)$row['account_id'])));
        foreach (array_keys($needles) as $needle) {
...
try {
    ...
    $accountPk = toggleResolveAccountPk(
        $pdo,
        $companyId,
    ...
    $current = getAccountStatus($pdo, $accountPk, $companyId);
    if (!$current) {
        api_error('无权限操作此账户', 403);
        exit;
    }
    $newStatus = $current['status'] === 'active' ? 'inactive' : 'active';
    updateAccountStatus($pdo, $newStatus, $accountPk);
